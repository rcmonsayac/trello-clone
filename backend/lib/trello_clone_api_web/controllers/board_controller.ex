defmodule TrelloCloneApiWeb.BoardController do
  ...
  plug(Resources, {:board, :board_permission} when action in [:show, :update, :delete])
  ...
  def create(%{assigns: assigns} = conn, params) do
    ...
    with {:ok, %Board{} = board} <- Boards.create_board(board_params),
         ...
         {:ok, %BoardPermission{} = _permission} <- Boards.create_board_permission(permission_params) |> IO.inspect do
      conn
      |> put_status(:created)
